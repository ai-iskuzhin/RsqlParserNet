...
Real database integration tests are intentionally not part of the default repository test suite yet. Before using RSQL on production-sensitive routes, add application-level coverage for the provider used by the application, such as PostgreSQL or SQL Server. SQL-generation tests catch translation failures; real database tests also catch provider collation, indexing, schema, and runtime behavior.
